 ...
 *   DENY / timeout / error / unparsable / tool-call output / abort / gate failure
...
/**
 * Gates 1-3: read the session and decide what this request asks to judge.
 * @returns the escalation under review, or `undefined` to defer to the user.
 */
function escalationOf(ctx, config, logger, req) {
  // Gate 1: session must sit in the configured preset right now.
  const presets = ctx.get('permissionPresets');
  if (presets === undefined) return undefined;
  const session = req.agent.session;
  if (presets.current(session) !== config.preset) return undefined;

  // Gate 2: only sandbox-escalation requests; everything else goes to the user.
  const reason = req.reason ?? '';
  if (!reason.startsWith(ESCALATION_PREFIX)) return undefined;
  ...
  return { call, events, reason, requestedMode };
}
...
export default {
  ...
  apply(ctx, config) {
    ...
    ctx.on('approval/request', async (req, next) => {
      // The gates read the session through the harness API. Any unexpected
      // failure there (harness API drift, missing projection) must defer to the
      // user: the approval service folds a throwing listener to 'unavailable',
      // which would deny the escalation outright instead of asking.
      let judged;
      try {
        judged = escalationOf(ctx, config, logger, req);
      } catch (error) {
        logger.warn('llm-approver: gate failed (%s); deferring to user', error?.message ?? String(error));
        return next();
      }
      if (judged === undefined) return next();
      const { call, events, reason, requestedMode } = judged;
